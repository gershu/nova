...
# Digest. Heute morgen will Stefan die ganze Chain nachholen, ohne 12
# launchctl-kickstarts.
#
# Vorgehen: ruft `python -m modules.<X> [subcommand]` DIREKT auf (synchron).
# NOVA_PARAMS_FILE-env-var pro Step gesetzt (Production-Convention) — Module
# lesen Default-Params von dort.
#
# Pre-Checks: IB-Gateway-Check vor IB-abhaengige Steps automatisch.
#
# Default-Chain (in Reihenfolge):
#   fred_ingest → market_monitor → ingest_fx → news_sa → ingest →
#   screener_csp → monitor → digest → alert_explainer →
#   portfolio_briefing → obsidian
#
...
# Chain-Definition: "label|module|subcommand_args|params_file"
# - module: passt zu 'python -m modules.<module>' (z.B. 'ingest', 'llm.alert_explainer')
# - subcommand_args: CLI-Args, kann leer sein
# - params_file: wird als NOVA_PARAMS_FILE env-var gesetzt; leer = keine Params
DAILY_CHAIN=(
    "fred_ingest|fred_ingest|fetch-all|"
    "market_monitor|market_monitor|run|"
    "ingest_fx|ingest_fx||${JOBS_DIR}/lab_ingest_fx_daily.json"
    "news_sa|news_sa|fetch|"
    "ingest|ingest||${JOBS_DIR}/lab_ingest_daily.json"
    "screener_csp|screener_csp|run|${JOBS_DIR}/lab_screener_csp_daily.json"
    "monitor|monitor||${JOBS_DIR}/lab_monitor_daily.json"
    "digest|digest||${JOBS_DIR}/lab_digest_daily.json"
    "alert_explainer|llm.alert_explainer||${JOBS_DIR}/lab_alert_explainer_daily.json"
    "portfolio_briefing|llm.portfolio_briefing||${JOBS_DIR}/lab_portfolio_briefing_daily.json"
    "obsidian|obsidian|publish|"
)
WEEKLY_CHAIN=(
    "fundamentals|fundamentals|refresh-all --since-days 6|"
    "screener_value|screener_value|filter|${JOBS_DIR}/lab_screener_value_weekly.json"
)
...
# Der Daily-Driver checkt das schon; wir delegieren nicht doppelt.
...
# Sanity-Check: venv-Python existiert?
if [[ ! -x "${PYTHON_BIN}" ]]; then
    echo "FEHLER: ${PYTHON_BIN} nicht executable. Erst node_deploy.sh laufen lassen." >&2
    exit 1
fi
...
echo "    Steps        (${#CHAIN[@]}):"
for entry in "${CHAIN[@]}"; do
    IFS='|' read -r label module args params_file <<< "${entry}"
    pf_short=""
    [[ -n "${params_file}" ]] && pf_short="  NOVA_PARAMS_FILE=$(basename "${params_file}")"
    echo "      ${label}  ->  python -m modules.${module} ${args}${pf_short}"
done
echo
...
run_step() {
    local label="$1" module="$2" args="$3" params_file="$4"
    ...
    # Direkter python -m Aufruf mit venv-Binary (kein PATH-Lookup noetig).
    ...
    (
        ...
        # ${args} bewusst unquotiert: erlaubt Mehrwort-Subcommands ('refresh-all --since-days 6')
        exec "${PYTHON_BIN}" -m "modules.${module}" ${args}
    ) > "${logfile}" 2>&1 || rc=$?
...
for entry in "${CHAIN[@]}"; do
    IFS='|' read -r label module args params_file <<< "${entry}"
    if run_step "${label}" "${module}" "${args}" "${params_file}"; then
